...
const portalNav = [
  ...
  { name: "Payments", href: "/portal/payments", icon: Banknote },
  { name: "Withdraw", href: "/portal/withdraw", icon: ArrowUpFromLine },
  { name: "Earn", href: "/portal/earn", icon: TrendingUp },
  { name: "Vesting", href: "/portal/vesting", icon: CalendarClock },
...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  }, [loading, authenticated, onboarded, user, router, pathname]);
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 bg-background border-r border-border p-4 flex flex-col">
            ...
            <PortalSidebarContent pathname={pathname} onNavigate={() => setSidebarOpen(false)} onLogout={logout} userName={user.name} />
          </div>
        </div>
      )}
